...
if(_action) then {
	_valid = [[_uid,_house],"TON_fnc_preCheckHouse",false,false] call life_fnc_MP;
 
	_valid_return = _house GVAR[format["preCheckHouse_%1",_uid],0];
	 
	if(EQUAL(_valid_return,0)) then
	{
		while{EQUAL(_valid_return,0)} do
		{
			sleep 0.2;
			_valid_return = _house GVAR[format["preCheckHouse_%1",_uid],0];
		};
	...
	if(EQUAL(_valid_return,true)) then {
